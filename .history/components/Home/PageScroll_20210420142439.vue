<template>
  ...
  >
    <slide v-for="(item, index) in items" :key="index">
      <v-container fluid>
        <v-row :id="item.carousel.content.href" class="white mr-5">
          <v-col cols="12">
            <v-img :src="item.section.image.url" class="img-fluid">
              ...
              <v-col cols="6">
                <v-container>
                  <v-row>
                    <transition @enter="enter" @before-enter="beforeEnter" appear>
                      <v-col
                        v-for="(service, index) in item.section.services"
                        :key="index"
                        cols="12"
                      >
                        <v-card dark delay="0" left="100">
                          <div
...
export default {
  ...
  methods: {
    ...
    enter(el, done) {
      const delay = parseInt(el.getAttribute('delay'))
      const left = parseInt(el.getAttribute('left'))
