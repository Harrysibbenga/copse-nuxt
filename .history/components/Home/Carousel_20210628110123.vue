    ...
    <v-carousel-item v-for="(item, i) in items" :key="i" class="error">
      <transition name="bounce">
        <v-row v-if="currentSlide == i" no-gutters>
          ...
          <v-responsive class="position-md-absolute">
            <v-card tile class="transparent py-10 px-6">
              <v-card-title>
                <h1
                  class="text-h4 text-md-h1 mx-auto white--text font-weight-medium"
                >
                  {{ item.carousel.content.title }}
                </h1>
              </v-card-title>
              <v-card-text class="white--text text-md-center text-center">
